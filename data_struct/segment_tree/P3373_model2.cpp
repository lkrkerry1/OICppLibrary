...
class LazySegmentTree
{
    ...
    constexpr inline int ls(const int x)
    {
        return x * 2 + 1;
    }
    constexpr inline int rs(const int x)
    {
        return x * 2 + 2;
    }
...
public:
    ...
    void set(const int p, const Lazy &k)
    {
        set(p, p + 1, k);
    }
    void set(const Lazy &k)
    {
        value[0] += k, lazy[0] += k;
    }
    ...
    LazySegmentTree(const int n)
        : LazySegmentTree(n, std::vector<Value>(n * 4)) {}
...
struct l
{
    long long x, y;
    bool flag;
    l(long long x = 0, long long y = 1) : x(x), y(y){};
...
{
    long long x, s;
    v(long long x = -lmax, long long s = 1) : x(x), s(s){};
    v operator+=(const l &rhs)
    {
        x = (x * rhs.y) % md;
        x = (x + s * rhs.x) % md;
        // cout << "v value: x: " << x << " s: " << s << endl;
        return *this;
    }
};
...
{
    return v((lhs.x + rhs.x) % md, lhs.s + rhs.s);
}
